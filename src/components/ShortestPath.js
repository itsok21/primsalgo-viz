...
export class ShortestPath extends Component {
    ...
    addWeights()
    {
        weights.push(this.state.pre_weight)
        this.forceUpdate()
    }
    ...
    changeWeight(e){
        this.reset()
        let x=document.getElementById('pre_weight').value
        this.setState({
            pre_weight:x
        })
    }
    ...
    render() {
        ...
        <div>
            <center>
            <h3>Prim's Minimal Spanning Tree Algorithm</h3>
            <button className="button button4" onClick={()=>this.prim(this.state.src)}>Prim's Algorithm</button>
            ...
            <text style={{fontFamily:'Georgia'}}>&nbsp;&nbsp;&nbsp;Weight value&nbsp;</text>
            <input type="text" id="pre_weight" value={this.state.pre_weight} style={{width:'25px'}} onChange={(e)=>this.changeWeight(e)} />
